...
from failed_category_list_url import failed_category_list_url
...
class Spider(threading.Thread):
    ...
    def getPaginationHtml(self, pageUrl, repeat_count=0):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                arcLinkTags1 = html.xpath("//div[@class='TypeList'][1]//a[@class='TypeBigPics']")
                arcLinkTags2 = html.xpath("//ul[@class='g-gxlist-imgbox'][1]//a")
                arcLinkTags = []
    ...
    def getArcHtml(self, arc_url, list_dir, repeat_count=0):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # 如果详情页有分页的情况
                if len(html.xpath("//div[@class='NewPages']/ul/li/a[text()='下一页']")) != 0:
            ...
            print("{}详情页下载异常，错误信息为{},错误行号为{}".format(self.threadName, str(e), e.__traceback__.tb_lineno))
            self.validip_que.get(validip)
            repeat_count += 1
            if repeat_count < 4:
    ...
    def downloadImage(self, src, file_path, repeat_count=0):
        ...
        try:
            ...
            print("{}图片下载异常，错误信息为{},异常行号为{}".format(self.threadName, str(e), e.__traceback__.tb_lineno))
            self.validip_que.get(validip)
            repeat_count += 1
            if repeat_count < 4:
    ...
    def sqlInsert(self, url, type):
        try:
            ...
            if self.mysqlClient.cursor().execute(sql):
                ...
                print("{}成功插入一条错误的{}记录{}到数据库".format(self.threadName, type, url))
        except BaseException as e:
...
def main():
    ...
    proxy_helper.run()
    time.sleep(10)
    url_list = failed_category_list_url
    url_que = Queue(5000)
