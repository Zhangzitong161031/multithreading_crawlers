...
class TranslateUtils(threading.Thread):
    def __init__(self,threadName,validip_que):
    ...
    def run(self,repeat_count=0):
        ...
        while True:
            en_img_info = self.redis.spop("en_img_info2")
    ...
    def startTranslate(self,en_img_info,repeat_count=0):
        ...
        if self.sendAjaxRequest():
            ...
            self.redis.sadd("cn_img_to_download",cn_img_info)
            print("{}成功翻译{}到{}".format(self.threadName, self.en_word, self.cn_word))
...
def main():
    ...
    time.sleep(10)
    for i in range(100):
        worker=TranslateUtils("翻译线程%d"%(i),validip_que)
        worker.start()
